...
namespace Lab8_Sets
{
    ...
    public class SetOperationsLab
    {
        ...
        public int DeduplicateEmails(List<string> emailList)
        {
            ...
            // store deduped emails in the field so other parts of the lab can use it
            uniqueEmails = deduped;

            // CLEAR the original list and re-copy unique emails
            emailList.Clear();
            foreach (var email in deduped)
            {
                emailList.Add(email);
            }
        ...
        public bool HasPermission(string userId, string permission)
        {
            ...
            HashSet<string> permissions = userPermissions[userId];
        ...
        public int AddPermissions(string userId, HashSet<string> newPermissions)
        {
            ...
            HashSet<string> currentPermissions = userPermissions[userId];

            int beforeCount = currentPermissions.Count;

            currentPermissions.UnionWith(newPermissions);

            int afterCount = currentPermissions.Count;
        ...
        public HashSet<string> GetMissingPermissions(string userId, HashSet<string> requiredPermissions)
        {
            ...
            if (!userPermissions.ContainsKey(userId))
            {
                // user does not exist, so they are missing everything required
                return new HashSet<string>(requiredPermissions);
            }

            HashSet<string> userSet = userPermissions[userId];

            // copy requiredPermissions so we do not change the original
            HashSet<string> missing = new HashSet<string>(requiredPermissions);

            // remove the ones the user already has
            missing.ExceptWith(userSet);
        ...
        public double CalculateRetentionRate()
        {
            ...
            double rate = (double)continuing.Count / enrolledLastQuarter.Count * 100.0;
